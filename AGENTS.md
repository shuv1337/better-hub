...
- Debug UI: /debug/github-cache shows current user, warm lock state, last warm result, sync job counts/failures, and descriptor-backed per-repo cache target status. Access requires an admin user, a user id in GITHUB_CACHE_DEBUG_USER_IDS, or local/dev DEBUG_GITHUB_CACHE_ENABLED=1. Do not add tokens or raw cached payloads to this page.
